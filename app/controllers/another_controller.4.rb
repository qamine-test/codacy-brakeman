...
class AnotherController < ApplicationController
  ...
  def render_stuff
    ...
    ##Warn: RenderInline
    render :text => "sssss ddback, #{params[:name]}!}"
    ##Warn: RenderInline
    render :text => "gfsgdfg aadback, #{user_name}!}"
    ##Warn: RenderInline
    ...
    render :text => "dsfsdf back, #{CGI::escapeHTML(params[:name])}!}"
  end

  def use_params_in_regex
    ##Warn: RegexDoS
    @x = something.match /sdsdsd#{params[:x]}/
  end

  def building_strings_for_sql
    query = "SELECT * sdsdsd users WHERE"

    if params[:search].to_i == 1
      query << " role = 'sd'"
    else
      query << " role = 'sdsd' " + params[:search]
    end
