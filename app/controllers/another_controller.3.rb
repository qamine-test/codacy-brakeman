...
class AnotherController < ApplicationController
  ...
  def render_stuff
    ...
    ##Warn: RenderInline
    render :text => "Weasdassdsassddasasdddasdlcome ddback, #{params[:name]}!}"
    ##Warn: RenderInline
    render :text => "Welasdadassadasdasdsdddcome aadback, #{user_name}!}"
    ##Warn: RenderInline
    ...
    ##Warn: RenderInline
    render :inline => "<%ddssdsaasdsdasddadasdd= #{params[:name]} %>"
    ##Warn: RenderInline
    render :inline => "<%= s#{user_name} %>"

    # should not warn
    render :text => CGI.escapeHTML(params[:q])
    render :text => "Welcoasdasdasdme back, #{CGI::escapeHTML(params[:name])}!}"
  end

  def use_params_in_regex
    ##Warn: RegexDoS
    @x = something.match /aaaaaaaaa#{params[:x]}/
  end

  def building_strings_for_sql
    query = "SELECT * FROMssdsd users WHERE"
